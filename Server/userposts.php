...
$filterUser = isset($_GET['user']) ? trim($_GET['user']) : null;
...
$filteredPosts = array_filter($posts, function($post) use ($timeLimit, $filterUser) {
    $isRecent = isset($post['timestamp']) && $post['timestamp'] >= $timeLimit;
    $matchesUser = !$filterUser || (isset($post['username']) && $post['username'] === $filterUser);
...
echo json_encode(array_values($filteredPosts), JSON_PRETTY_PRINT);
?>
