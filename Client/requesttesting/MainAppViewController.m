...
@interface MainAppViewController () <UIAlertViewDelegate, UITextFieldDelegate, UIScrollViewDelegate>
@end
...
- (void)viewDidLoad {
    ...
    self.likedPosts = [[NSMutableArray alloc] init];
    
    self.postViews = [[NSMutableArray alloc] init];
    
    //CGFloat topMargin = 0;
    //self.scrollView.contentInset = UIEdgeInsetsMake(topMargin, 0, 0, 0);
    //self.scrollView.scrollIndicatorInsets = UIEdgeInsetsMake(topMargin, 0, 0, 0);
    ...
    if (currentUsername) {
        //self.titlebar.topItem.title = [NSString stringWithFormat:@"%@", currentUsername];
    } else {
...
- (void)fetchPosts {
    ...
    for (NSDictionary *dict in jsonArray) {
        ...
        NSTimeInterval ts = 0;
        if ([[dict objectForKey:@"timestamp"] respondsToSelector:@selector(doubleValue)]) {
            ts = [[dict objectForKey:@"timestamp"] doubleValue];
        }
        NSString *formattedTime = @"";
        if (ts > 0) {
            NSDate *date = [NSDate dateWithTimeIntervalSince1970:ts];
            NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
            formatter.dateFormat = @"MMM d, HH:mm"; // Example: Aug 2, 3:15 PM
...
- (void)showPostDetail:(UIButton *)sender {
    ...
    NSDictionary *postDict = @{
                               @"id": @(pv.tag),
                               @"username": pv.authorBtn.accessibilityLabel ?: @"",
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == alertView.firstOtherButtonIndex) {
        ...
        if (username && text.length > 0) {
            ...
            [self showUIPopup:@"..." :@"Post can't be blank."];
        }
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    CGFloat pullThreshold = -40; 
    
    if (scrollView.contentOffset.y < pullThreshold) {
        [self fetchPosts];
        NSLog(@"Refresh triggered");
        [self showUIPopup:@"Feed Refreshed" :@""];
    }
}

- (void)viewDidUnload {
    [self setPostTypeSelector:nil];
    [super viewDidUnload];
}
@end
