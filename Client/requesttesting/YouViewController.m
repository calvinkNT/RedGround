...
#import "SetBG.h"
@interface YouViewController () <UIAlertViewDelegate, UITextFieldDelegate, UIScrollViewDelegate>
...
- (void)viewDidLoad {
    ...
    if ([[NSUserDefaults standardUserDefaults] stringForKey:@"username"].length > 0) {
        self.usernameLabel.text = [NSString stringWithFormat: @"%@'s posts", [[NSUserDefaults standardUserDefaults] stringForKey:@"username"]];
        NSString *escapedUser = [[[NSUserDefaults standardUserDefaults] stringForKey:@"username"] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
- (IBAction)postTypeChanged:(UISegmentedControl *)sender {
    ...
    [self fetchPosts]; // reload immediately
}
...
- (void)likeTapped:(UIButton *)sender {
    ...
    if (![container isKindOfClass:[PostView class]]) return;
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == alertView.firstOtherButtonIndex) {
        ...
        if (username && text.length > 0) {
            ...
            [self showUIPopup:@"..." :@"Post can't be empty"];
        }
...
- (IBAction)settingsBtn:(id)sender {
    ...
    SettingsViewController *detailVC = [storyboard instantiateViewControllerWithIdentifier:@"SettingsViewController"];
    [self presentViewController:detailVC animated:YES completion:nil];
